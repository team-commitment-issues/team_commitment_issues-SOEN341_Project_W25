...
describe('POST /channel/addUserToChannel', () => {
  it('should add a user to a channel successfully', async () => {
    ...
    const channel = await TestHelpers.createTestChannel(
      'Test Channel',
      team._id,
      superAdminUser._id,
      [teamMember._id],
...
describe('POST /channel/removeUserFromChannel', () => {
  it('should remove a user from a channel successfully', async () => {
    ...
    const updatedChannel = await Channel.findOne({ name: channel.name }).populate('members');
    expect(updatedChannel?.members).not.toContain(user._id);

    const updatedTeamMember = await TeamMember.findOne({ user: user._id, team: team._id });
...
describe('POST /channel/deleteChannel', () => {
  it('should delete a channel successfully', async () => {
    ...
    const teamMember = await TestHelpers.createTestTeamMember(user._id, team._id, TeamRole.ADMIN, [
      channel._id
    ]);
...
describe('POST /channel/deleteMessage', () => {
  it('should delete a message successfully', async () => {
    ...
    const updatedChannel = await Channel.findOne({ name: channel.name }).populate('messages');
