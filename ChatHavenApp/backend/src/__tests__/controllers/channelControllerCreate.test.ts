...
describe('POST /channel/createChannel', () => {
  ...
  it('should return an error if the user is not authorized', async () => {
    ...
    const response = await request(app).post('/channel/createChannel').send(newChannel).expect(401);
  ...
  it('should add all selected team members (users) to the channel', async () => {
    ...
    const teamMember1 = await TestHelpers.createTestTeamMember(
      user1._id,
      team._id,
      TeamRole.MEMBER,
      []
    );
    const teamMember2 = await TestHelpers.createTestTeamMember(
      user2._id,
      team._id,
      TeamRole.MEMBER,
      []
    ...
    const response = await request(app)
      ...
      .send({
        teamName: team.name,
        channelName: name,
        selectedTeamMembers: [user1.username, user2.username]
      })
      .expect(201);
