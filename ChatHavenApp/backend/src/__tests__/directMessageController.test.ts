...
describe('POST /directMessage/getDirectMessages', () => {
    it('should return all direct messages between two users', async () => {
        ...
        const receiver = await TestHelpers.createTestUser('recipient@user.com', 'testpassword', 'Recipient', 'User', 'recipientuser', Role.USER, []);
        ...
        const dmessage2 = new DMessage({ text: 'Test message 2', username: receiver.username, directMessage: directMessage._id });
