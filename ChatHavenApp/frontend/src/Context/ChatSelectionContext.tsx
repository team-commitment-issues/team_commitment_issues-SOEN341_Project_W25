...
export const ChatSelectionProvider: React.FC<ChatSelectionProviderProps> = ({ children }) => {
  ...
  const validateTeam = useCallback(async (teamName: string): Promise<boolean> => {
  ...
  const validateChannel = useCallback(async (teamName: string, channelName: string): Promise<boolean> => {
  ...
  const isValidSelection = useCallback(async (sel: Selection): Promise<boolean> => {
