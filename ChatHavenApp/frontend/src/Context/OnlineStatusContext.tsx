...
export const OnlineStatusProvider: React.FC<OnlineStatusProviderProps> = ({ children }) => {
  ...
  const getUserStatus = useCallback(
    (username: string): UserStatus | undefined => {
  ...
  const updateUserStatus = useCallback(
    (username: string, status: Status, lastSeen?: Date) => {
      setOnlineUsers(prev => ({
        ...prev,
  ...
  const refreshStatuses = useCallback(
    async (usernames: string[]): Promise<void> => {
  ...
  const subscribeToTeamStatuses = useCallback(
    (teamName: string) => {
  ...
  const subscribeToChannelStatuses = useCallback(
    (teamName: string, channelName: string) => {
      ...
      try {
        ...
        console.error(
          `Failed to subscribe to channel statuses for ${teamName}/${channelName}:`,
          error
        );
      }
    },
