...
export const updateUsername = async (oldUsername: string, newUsername: string, password: string) => {
    try {
        ...
        throw new Error((error as any).response?.data?.error || 'Username update failed. Please try again.');
    }
};
...
export const updatePassword = async (oldPassword: string, newPassword: string) => {
    try {
        ...
        throw new Error((error as any).response?.data?.error || 'Password update failed. Please try again.');
    }
};